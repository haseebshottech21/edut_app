...
class UpdateUserProfile extends Component {
  ...
  _onSubmit = () => {
    ...
    if (this._validateForm() && !isTherapist) {
      ...
      if (!_.isEmpty(userImage) && imageSelected == true) {
        const payload = {
    ...
    try {
      ...
      launchCamera(options, response => {
        ...
        if (response.didCancel) {
          ...
          const source = response.assets && response.assets !== undefined && response.assets[0];
  ...
  render() {
    ...
    console.log(this.state.userImage, 'checkkkk');
    return (
...
export default connect(mapStateToProps, actions)(UpdateUserProfile);
