...
class TherapistSignUp extends Component {
  ...
  selectItem = (item, count) => {
    ...
    const Index = _.findIndex(this.state.serverArray, o => o.key === item.key);
  ...
  renderDays = ({item, index}) => {
    let Index = _.findIndex(this.state.serverArray, o => o.key === item.key);
...
export default connect(mapStateToProps, actions)(TherapistSignUp);
